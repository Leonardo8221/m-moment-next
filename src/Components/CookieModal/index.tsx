...
const Modal = ({ onClose, cookieName }: any) => {
  ...
  const handleRejectAll = () => {
    setCookie(cookieName, "false", { maxAge: 365 * 24 * 60 * 60 });
    onClose();
  };
  ...
  return (
    <div className={`w-[690px] h-[670x] ${styles.cookieModal}`}>
      ...
      <div className="p-5 px-0 rounded-[10px] h-full bg-white relative">
        ...
        <div
          className={`relative -top-[40px] overflow-auto  ${styles.modalBody}`}
        >
          <div>
            <h1 className="text-[--blue] font-[DMSans] text-[17px] font-[500]">
              Cookie Usage
            </h1>
          ...
          <div className={styles.accordions}>
            {CookieMap.map((item, index) => (
              ...
              >
                ...
                {index === expandedIndex && item.id === "002" && (
                  <div>
        ...
        <div
          className={`flex items-center justify-between gap-4 w-full ${styles.modalFooter}`}
        >
          <div className="flex items-center gap-4">
            <button
              className="bg-[--blue] py-2 px-4 rounded-md text-white !transition-all !duration-500 !ease-in-out hover:!bg-[--hoverblue]"
              onClick={handleAcceptAll}
            >
              Accept All
            </button>
            <button
              className="bg-[#F5F5F6] py-2 px-4 rounded-md text-[--dark] !transition-all !duration-500 !ease-in-out hover:!bg-[#363c4f29]"
          ...
          <button
            className="bg-[#F5F5F6] hover:!bg-[#363c4f29] !transition-all !duration-500 !ease-in-out py-2 px-4 rounded-md text-[--dark]"
